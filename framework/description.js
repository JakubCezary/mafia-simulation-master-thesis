var oes = oes || {}, dom = dom || {};
var sim = sim || {};
//var scenario = {model:{space:{},v:{},f:{}},initialState:{}};
...
oes.ui.createBackToSimulationLinks = function () {
  ...
  backLinkEl.href = "simulation.html";
  backLinkEl.textContent = "Back to simulation";
...
oes.ui.setupDescription = function () {
  var el=null, refEl=null, pageTitleEl=null, mainSectElems=null, explReqElems=null, i=0,
  ...
  var simName = String( sim.scenario.name || sim.model.name),
  ...
  // set HTML title
  if (!document.title) document.title = "OES "+ simName;
  // create page heading
  pageTitleEl = document.querySelector("body > header > h1");
  if (!pageTitleEl) {
    pageTitleEl = document.createElement("h1");
    header.insertBefore( pageTitleEl, header.firstElementChild);
  }
  ...
  el.innerHTML = "Model name: "+ simName +
      ", available on <a href='http://sim4edu.com'>Simulation-for-Education</a>.";
  dom.insertAfter( el, pageTitleEl);
  refEl = el;
  if (sim.model.source) {
    el = document.createElement("p");
    el.innerHTML = "Based on "+ sim.model.source;
  ...
  el.textContent = "Model Description";
  ...
  if (sim.model.systemNarrative) {
    if (sysNarr) sysNarr.innerHTML = sim.model.systemNarrative;
      ...
      el = document.createElement("h1");
      el.textContent = "System Narrative";
  ...
  for (i=0; i < mainSectElems.length; i++) {
    mainSectElems[i].firstElementChild.addEventListener("click", function (e) {
  ...
  for (i=0; i < explReqElems.length; i++) {
    explReqElems[i].addEventListener("mouseover", function (e) {
      ...
      if (!explEl.innerHTML) explEl.innerHTML = oes.ui.explanation[explEl.id];
  ...
  oes.ui.createBackToSimulationLinks();
};
