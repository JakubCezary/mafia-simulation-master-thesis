...
oes.loadManager.showConfigInConsole = function() {
  var compSrc = oes.loadManager.components,
  ...
  console.log("pageType = '" + oes.loadManager.pageType + "'");
 ...
 * and pageType (one of "simulation" or "description".
 */
oes.loadManager.setup = function(callback) {
  ...
  // detect if the page is "simulation" or description , which either contain
  // description.html at end or the "description" word as part of URL,
  // this being for rest like style, that can be used on the server.
  // NOTE: defaults to "simulation"
  if (url.indexOf("description") !== -1
    || url.indexOf("description.html") !== -1)
    oes.loadManager.pageType = "description";
  else oes.loadManager.pageType = "simulation";
  ...
  oes.loadManager.loadFile("loadManagerConfig.js", oes.loadManager._ownPath, function() {
    ...
    oes.loadManager.codeLoadingMode = "dev";
    // update progress bar
    oes.loadManager.updateProgressBar(10);
    // finished, now call the callback
    callback();
...
oes.loadManager.preload = function (callback) {
  ...
  // for multi-scenarios case, the path is one level deeper than for single scenarios
  if (oes.loadManager.pageType === "simulation") {
    ...
    oes.loadManager.loadFileBatch(compSrc.coreCssSrc, basePath, function () {
      // load simulator JS core
      oes.loadManager.loadFileBatch(compSrc.coreJsSrc, basePath, function () {
        // add the script element for scenario.js
        oes.loadManager.progressIncStep = oes.loadManager.getProgressBarValue() / 2 ;
        oes.loadManager.loadFile("scenario.js", scenarioFilePath, function (scenarioScriptEl) {
          oes.loadManager.loadScenarioPrerequisites(basePath, function () {
            // remove the script element containing scenario.js
            ...
            // add the script element for scenario.js
            oes.loadManager.loadFile("scenario.js", scenarioFilePath, function () {
              setTimeout( function () {
  ...
  } else if (oes.loadManager.pageType === "description") {
    // no progress bar for the description.html case.
    oes.loadManager.removeProgressbar();
    oes.loadManager.loadFileBatch( compSrc.description, basePath, function () {
      // add the script element for scenario.js
      oes.loadManager.loadFile("scenario.js", "./", function() {
        // clear the browser console.
        oes.loadManager.clearConsole();
        // call frontend initialization
        oes.ui.setupDescription();
        // callback at the end of preloading
        oes.loadManager.loadFinishedCallback();
      }, function () {
        // single model multiple scenarios, loading default description
        // which belongs to the model and not to the scenario
        // add the script element for model.js
        oes.loadManager.loadFile("model.js", "./", function() {
          // clear the browser console.
          oes.loadManager.clearConsole();
          // call frontend initialization
          oes.ui.setupDescription();
...
/**
 * Investigate the scenario.js file and detects which of the additional
...
oes.loadManager.loadScenarioPrerequisites = function (basePath, callback) {
  ...
  if (!sim.scenario) throw "The 'scenario.js' file could not be loaded or it contains errors!";
  ...
  if (sim.scenario.observationUI.spaceView.type) {
    if (!sim.model.space.type) {
      // the space core needs to be loaded
      compToLoad = compToLoad.concat(compSrc.coreSpace);
    }
    // detect visualization module
    if (sim.scenario.observationUI.spaceView.type && sim.scenario.visualize) {
  ...
  if (isGridSpace && !detectedVisualisation && sim.scenario.visualize)
...
oes.loadManager.createProgressBar = function () {
  ...
  progressContainer.append(progressTitle);
